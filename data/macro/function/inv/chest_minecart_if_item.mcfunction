...
#   data modify storage macro:input callback     set value "setblock ~ ~1 ~ minecraft:air"
#   function macro:inv/chest_minecart_if_item with storage macro:input {}
# ============================================

$execute as @a at @s if items entity @e[type=minecraft:chest_minecart,distance=..2,limit=1,tag=$(tag)] container.$(slot) *[minecraft:custom_data=$(customData)] run $(callback)
