...
# ─── Global storage init ───────────────────────────────
execute unless data storage macro:engine global run data modify storage macro:engine global set value {version:"V1.0.5-PRE4"}
...
execute unless data storage macro:engine global{version:"V1.0.5-PRE4"} run data modify storage macro:names temp.vers set value "V1.0.5-PRE3"
execute unless data storage macro:engine global{version:"V1.0.5-PRE4"} run tellraw @a [{"text":"[AME] ","color":"aqua","bold":true},{"text":"Version mismatch — expected ","color":"red"},{"storage":"macro:names","nbt":"temp.vers","color":"yellow"},{"text":".","color":"red"}]
execute unless data storage macro:engine global{version:"V1.0.5-PRE4"} run return 0
...
execute if score $ame_ver_set ame.pre_version matches 1 run execute unless score $v_patch ame.pre_version matches 5 run scoreboard players set $ver_mismatch ame.pre_version 1
