...
setblock -30000000 0 1600 minecraft:test_block[mode=log]{mode:"log",message:"❌ [AME] Version conflict detected. Expected v1.0.5. Run /reload in-game for details."}

# ─── Broadcast warning (all players) ──────────────────────
tellraw @a ["",{"text":"❌ ","color":"red"},{"text":"[AME] ","color":"aqua","bold":true},{"text":"Version conflict! ","color":"red","bold":true},{"text":"Expected ","color":"gray"},{"text":"v1.0.5","color":"yellow","bold":true},{"text":" — stored scores do not match.","color":"gray"}]
...
# ─── AME log buffer (WARN) ────────────────────────────────
data modify storage macro:input message set value "❌ Version mismatch — expected v1.0.5. Load aborted."
